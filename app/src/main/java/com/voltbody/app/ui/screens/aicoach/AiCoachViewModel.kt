...
class AiCoachViewModel @Inject constructor(
    ...
    init {
        ...
        val profile = appRepository.profile.value
    ...
    fun sendMessage(text: String = _uiState.value.inputText.trim()) {
        ...
        viewModelScope.launch {
            val token = appRepository.authToken.value
            if (token == null) {
            ...
            val allLogs = appRepository.workoutLogs.value
            ...
            runCatching {
                ...
                replaceThinkingWithError(NetworkErrorMapper.parse(e))
            }
        }
    }
    ...
    fun clearChat() {
        ...
        // Re-add greeting
        val name = appRepository.profile.value?.name?.takeIf { it.isNotBlank() } ?: "atleta"
        _uiState.value = _uiState.value.copy(
            messages = listOf(
                ChatMessage(
                    id = "greeting",
